...
# Error: out of bounds since seq[i] is value at position, not the position itself
# for i in seq: -> goes through each ELEMENT of seq, so i is appropriate
# could either change iterable (line 3) or line 4 (what we did) since error is disconnect between lines 3 and 4
...
"""
#list comprehension: [[int(percent * max_score) for percent in percentages]
# same as for percent in percentages: 
...
""" Rewrite that line to correct the error
    Please keep the code in quotes!"""
print("The line should be: ", "[int(percent * max_score) for percent in percentages]")
...
#apostrophe interferes with quotations
# patients[0] keeps using data from first patient
